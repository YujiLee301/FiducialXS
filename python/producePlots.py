...
def parseOptions():
    ...
    # input options
    parser.add_option('-d', '--dir',    dest='SOURCEDIR',  type='string',default='./datacardInputs/allYear/', help='run from the SOURCEDIR as working area, skip if SOURCEDIR is an empty string')
    ...
    parser.add_option('',   '--obsName',dest='OBSNAME',    type='string',default='pT4l',   help='Name of the observalbe, supported: "inclusive", "pT", "eta", "Njets"')
    ...
    parser.add_option('',   '--setLog', action='store_true', dest='SETLOG', default=True, help='set plot to log scale y, default is False')
    parser.add_option('',   '--unblind', action='store_true', dest='UNBLIND', default=False, help='Use real data')
    parser.add_option('',   '--lumiscale', type='string', dest='LUMISCALE', default='1.0', help='Scale yields')
    parser.add_option('-y', '--year', dest="ERA", type = 'string', default = 'allYear', help='Specifies the data taking period')
...
#   resultsXS_LHScan_observed_mass4l_v3.py
    if (obsName == "mass4l"):
        ...
        #_temp = __import__('resultsXS_'+ext+obsName+'_v3'+floatfix, globals(), locals(), ['modelNames', 'asimovDataModelName', 'resultsXS', 'modelIndUncert'], -1)
        #modelNames = _temp.modelNames
        #asimovDataModelName = _temp.asimovDataModelName
        #modelIndUncert = _temp.modelIndUncert
    ...
    for obsBin in range(nBins):
        ...
        if (opt.UNBLIND):
            data[obsBin] = resultsXS[datamodel+"_"+obsName+"_genbin_"+str(obsBin)]["central"]
            data_hi[obsBin] = resultsXS[datamodel+"_"+obsName+"_genbin_"+str(obsBin)]["uncerUp"]
            data_lo[obsBin] = -1.0*resultsXS[datamodel+"_"+obsName+"_genbin_"+str(obsBin)]["uncerDn"]
        ...
        if (opt.UNBLIND):
            ...
            systematics_hi[obsBin] = sqrt(max(0.0,resultsXS["SM_125_"+obsName+"_genbin_"+str(obsBin)]["uncerUp"]**2-resultsXS["SM_125_"+obsName+"_genbin_"+str(obsBin)+'_statOnly']["uncerUp"]**2))
            systematics_lo[obsBin] = sqrt(max(0.0,resultsXS["SM_125_"+obsName+"_genbin_"+str(obsBin)]["uncerDn"]**2-resultsXS["SM_125_"+obsName+"_genbin_"+str(obsBin)+'_statOnly']["uncerDn"]**2))
    ...
    if (obsName=="mass4l"):
        ...
        elif (obsName=="pt_leadingjet_pt30_eta4p7"): offset=30.0
        ...
        print (a_observable)
        print (a_observable_1)
        print (a_observable_2)
        print (a_observable_3)
        ...
        if obsName.startswith('pt_leading') or obsName.startswith('mj1j2'):
            v_ggH_powheg[0] = v_ggH_powheg[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_powheg_unc_hi[0] = v_ggH_powheg_unc_hi[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_powheg_unc_lo[0] = v_ggH_powheg_unc_lo[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_minloHJ[0] = v_ggH_minloHJ[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_minloHJ_unc_hi[0] = v_ggH_minloHJ_unc_hi[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_minloHJ_unc_lo[0] = v_ggH_minloHJ_unc_lo[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_mad[0] = v_ggH_mad[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_mad_unc_hi[0] = v_ggH_mad_unc_hi[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_ggH_mad_unc_lo[0] = v_ggH_mad_unc_lo[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_XH[0] = v_XH[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_XH_hi[0] = v_XH_hi[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_XH_lo[0] = v_XH_lo[0] * (float(obs_bins[1])-float(obs_bins[0]))
            a_XH[0] = a_XH[0] * (float(obs_bins[1])-float(obs_bins[0]))
            a_XH_hi[0] = a_XH_hi[0] * (float(obs_bins[1])-float(obs_bins[0]))
            a_XH_lo[0] = a_XH_lo[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_data[0] = v_data[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_data_hi[0] = v_data_hi[0] * (float(obs_bins[1])-float(obs_bins[0]))
            v_data_lo[0] = v_data_lo[0] * (float(obs_bins[1])-float(obs_bins[0]))
    ...
    if (obsName=="mass4l"):
        ...
        if ("jet" in obsName and (not (obsName.startswith("njets") or obsName.startswith("pt_leading")))):
    ...
    g_ggH_mad = TGraphAsymmErrors(v_observable_3,v_ggH_mad,v_dobservable_2,v_dobservable_2,v_ggH_mad_unc_lo,v_ggH_mad_unc_hi)
    ...
    g_ggH_madBorder = TGraphAsymmErrors(v_observable_3,v_ggH_mad,v_dobservable_2,v_dobservable_2,v_ggH_mad_unc_lo,v_ggH_mad_unc_hi)
    ...
    v_ratio_mad = TVectorD(len(ggH_mad), array('d',[ggH_mad[i]/ggH_minloHJ[i] for i in range(len(ggH_mad))]))
    v_ratio_mad_hi = TVectorD(len(ggH_mad), array('d',[ggH_mad_NNLOunc_hi[i]/ggH_minloHJ[i] for i in range(len(ggH_mad))]))
    v_ratio_mad_lo = TVectorD(len(ggH_mad), array('d',[ggH_mad_NNLOunc_lo[i]/ggH_minloHJ[i] for i in range(len(ggH_mad))]))
    ...
    g_ratio_mad = TGraphAsymmErrors(v_observable_3,v_ratio_mad,v_dobservable_2,v_dobservable_2,v_ratio_mad_lo,v_ratio_mad_hi)
    ...
    g_ratio_madBorder = TGraphAsymmErrors(v_observable_3,v_ratio_mad,v_dobservable_2,v_dobservable_2,v_ratio_mad_lo,v_ratio_mad_hi)
    ...
    with open(opt.inYAMLFile, 'r') as ymlfile:
        ...
        if (obsName.startswith('pt_leading')): label = 'p_{T,j1}'
    ...
    if(opt.SETLOG):
        ...
        if obsName == 'rapidity4l': dummy.SetMaximum(100.0*max(max(a_data),(max(a_ggH_powheg))))
    else:
    ...
    if (opt.SETLOG and (obsName.startswith('njets') or obsName.startswith('pt_leading'))):
       ...
       # dummy.GetXaxis().SetRangeUser(0,250)
    else:
        dummy.GetXaxis().SetTitle("")
    ...
    if (obsName=="pT4l") and ('njets' in obsName) and ('rapidity4l' in obsName):
        box = TPaveText(240,-0.4,260,-0.1)
        box.SetLineColor(0)
        box.SetFillColor(0)
        box.AddText("   ")
        box.Draw("same")
        #box1 = TPaveText(200,-0.4,220,-0.1)
        #box1.SetLineColor(0)
        #box1.SetFillColor(0)
        #box1.AddText("   ")
        #box1.Draw("same")
