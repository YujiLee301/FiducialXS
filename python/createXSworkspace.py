...
def createXSworkspace(obsName, channel, nBins, obsBin, observableBins, usecfactor, addfakeH, modelName, physicalModel, year, obs_ifJES, obs_ifJES2, obs_ifAbs, obs_ifAbs2, zzFloatType = ''):
    ...
    if len(obsNameOrig)>1:
        ...
        if (ifLLR):
            if obsName == 'dEtaj1j2': SuffixOfRootFile = SuffixOfRootFile.replace("ZZ","zz")
            else:  SuffixOfRootFile = SuffixOfRootFile.replace("ZZ","zz").replace(".0","")
	...
	print "obsBin_low:  ", obsBin_low
        ...
        SuffixOfRootFile = obsNameOrig[0]+"_"+obsBin_low+"_"+obsBin_high
        if (ifLLR) and not ((obsName == 'rapidity4l') or (obsName == 'dEtaj1j2')): SuffixOfRootFile = SuffixOfRootFile.replace("ZZ","zz").replace(".0","")
        if (ifLLR) and ((obsName == 'rapidity4l') or (obsName == 'dEtaj1j2')) : SuffixOfRootFile = SuffixOfRootFile.replace("ZZ","zz")
    ...
    for genbin in range(nBins):
        ...
        trueH_shape[genbin].SetName("trueH"+channel+"Bin"+str(genbin)) #TEST for mass4l is without bin0
        if physicalModel == 'v3':
        #if physicalModel == 'v2': #FIXME: yuji debug attempt
             trueH_shape[genbin].SetName("trueHBin"+str(genbin))
             #trueH_shape[genbin].SetName("trueH")
    ...
    for genbin in range(nBins):
        ...
        if (physicalModel=="v3"):
            ...
            if( not (obs_ifJES) or (not doJES)) :
                ...
                #Mass4l test
                trueH_norm[genbin] = RooFormulaVar("trueHBin"+str(genbin)+"_norm","@0*@1*@2", RooArgList(SigmaHBin[channel+str(genbin)], fideff_var[genbin], lumi) );
                #trueH_norm[genbin] = RooFormulaVar("trueH_norm","@0*@1*@2", RooArgList(SigmaHBin[channel+str(genbin)], fideff_var[genbin], lumi) );
            else :
                trueH_norm[genbin] = RooFormulaVar("trueHBin"+str(genbin)+"_norm","@0*@1*(1-@2)", RooArgList(fideff_var[genbin], lumi, JES_sig_rfv) )

            logger.debug("obs_ifJES: {},  doJES: {}, condition: {}".format(obs_ifJES, doJES, (not (obs_ifJES) or (not doJES))))
            if (obs_ifJES) and (doJES):
            #if (obs_ifJES):
                trueH_norm_final[genbin] = RooFormulaVar("trueHBin"+str(genbin)+recobin+"_final","@0*@1*@2*(1-@3)" ,RooArgList(SigmaHBin[channel+str(genbin)],fideff_var[genbin],lumi,JES_sig_rfv))
            else:
                trueH_norm_final[genbin] = RooFormulaVar("trueHBin"+str(genbin)+recobin+"_final","@0*@1*@2" ,RooArgList(SigmaHBin[channel+str(genbin)],fideff_var[genbin],lumi))
    ...
    if ifLLR:
            bkg_sample_tags = { 'qqzz':{'2e2mu':'qqzz', '4e':'qqzz', '4mu':'qqzz'},
                         'ggzz':{'2e2mu':'ggzz', '4e':'ggzz', '4mu':'ggzz'},
                         'zjets':{'2e2mu':'ZJetsCR', '4e':'ZJetsCR', '4mu':'ZJetsCR'}}

    if ifLLR:
        if (obsName == 'mj1j2'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_mjj_'+recobin]
        elif (obsName == 'pt_leadingjet_pt30_eta4p7'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_pTj1_'+recobin]
        elif (obsName == 'dEtaj1j2'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_absdetajj_'+recobin]
        elif (obsName == 'mass4lj'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_mHj_'+recobin]
        elif (obsName == 'pT4lj'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_pTHj_'+recobin]
        elif (obsName == 'pT4ljj'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_pTHjj_'+recobin]
        elif (obsName == 'TauB_Inc_0j_pTWgt'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_TBjmax_'+recobin]
        elif (obsName == 'TauC_Inc_0j_EnergyWgt'):
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_TCjmax_'+recobin]
        else:
            frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_'+obsNameDictKey+'_'+recobin]
           
    else:   frac_qqzz = fractionsBackground[bkg_sample_tags['qqzz'][channel]+'_'+channel+'_'+obsNameDictKey+'_'+recobin]
    frac_qqzz_var  = RooRealVar("frac_qqzz_"+recobin+"_"+channel+"_"+year,"frac_qqzz_"+recobin+"_"+channel+"_"+year, frac_qqzz);

    if ifLLR:
        if (obsName == 'mj1j2'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_mjj_'+recobin]
        elif (obsName == 'pt_leadingjet_pt30_eta4p7'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_pTj1_'+recobin]
        elif (obsName == 'dEtaj1j2'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_absdetajj_'+recobin]
        elif (obsName == 'mass4lj'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_mHj_'+recobin]
        elif (obsName == 'pT4lj'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_pTHj_'+recobin]
        elif (obsName == 'pT4ljj'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_pTHjj_'+recobin]
        elif (obsName == 'TauB_Inc_0j_pTWgt'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_TBjmax_'+recobin]
        elif (obsName == 'TauC_Inc_0j_EnergyWgt'):
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_TCjmax_'+recobin]
        else:
            frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_'+obsNameDictKey+'_'+recobin]

    else:   frac_ggzz = fractionsBackground[bkg_sample_tags['ggzz'][channel]+'_'+channel+'_'+obsNameDictKey+'_'+recobin]
    ...
    if ifLLR:
        if (obsName == 'mj1j2'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_mjj_'+recobin]
        elif (obsName == 'pt_leadingjet_pt30_eta4p7'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_pTj1_'+recobin]
        elif (obsName == 'dEtaj1j2'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_absdetajj_'+recobin]
        elif (obsName == 'mass4lj'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_mHj_'+recobin]
        elif (obsName == 'pT4lj'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_pTHj_'+recobin]
        elif (obsName == 'pT4ljj'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_pTHjj_'+recobin]
        elif (obsName == 'TauB_Inc_0j_pTWgt'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_TBjmax_'+recobin]
        elif (obsName == 'TauC_Inc_0j_EnergyWgt'):
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+'_TCjmax_'+recobin]
        else:
            frac_zjets = fractionsBackground[bkg_sample_tags['zjets'][channel]+"_"+channel+"_"+obsNameDictKey+'_'+recobin]
    ...
    if ((ifLLR) and (obsName == "njets_pt30_eta4p7")) : SuffixOfRootFile = SuffixOfRootFile.replace('_10', '_14')
    if ((ifLLR) and (obsName == "mj1j2")) : SuffixOfRootFile = SuffixOfRootFile.replace('_-1000', '_-2').replace('inf', '1300').replace('mj1j2', 'mjj')
    if ((ifLLR) and (obsName == "pt_leadingjet_pt30_eta4p7")) : SuffixOfRootFile = SuffixOfRootFile.replace('13000', '1300').replace('pt_leadingjet_pt30_eta4p7', 'pTj1')
    if ((ifLLR) and (obsName == "pTj2")) : SuffixOfRootFile = SuffixOfRootFile.replace('inf', '1300').replace('-11','-2')
    if ((ifLLR) and (obsName == "dEtaj1j2")) : SuffixOfRootFile = SuffixOfRootFile.replace('inf', '10.0').replace('-10000','-100.0').replace('dEtaj1j2', 'absdetajj')
    if ((ifLLR) and (obsName == "mass4lj")) : SuffixOfRootFile = SuffixOfRootFile.replace('-11','-2').replace('mass4lj', 'mHj')
    if ((ifLLR) and (obsName == "pT4lj")) : SuffixOfRootFile = SuffixOfRootFile.replace('pT4lj', 'pTHj').replace('inf', '1300')
    if ((ifLLR) and (obsName == "pT4ljj")) : SuffixOfRootFile = SuffixOfRootFile.replace('pT4ljj', 'pTHjj').replace('inf', '1300')
    if ((ifLLR) and (obsName == "TauB_Inc_0j_pTWgt")) : SuffixOfRootFile = SuffixOfRootFile.replace('inf', '1000').replace('TauB_Inc_0j_pTWgt', 'TBjmax').replace('-10000','-2')
    if ((ifLLR) and (obsName == "TauC_Inc_0j_EnergyWgt")) : SuffixOfRootFile = SuffixOfRootFile.replace('inf', '1000').replace('TauC_Inc_0j_EnergyWgt', 'TCjmax').replace('-10000','-2')


    logger.debug("SuffixOfRootFile: "+SuffixOfRootFile)
    print ("SuffixOfRootFile: "+SuffixOfRootFile)
    ...
    if (ifLLR): template_qqzzName = template_qqzzName.replace("ZZ","zz").replace('13000','3000')
    qqzzTempFile = TFile(template_qqzzName,"READ")
    if (not ifLLR): qqzzTemplate = qqzzTempFile.Get("m4l_"+SuffixOfRootFile)
    if (ifLLR) and not((obsName == "rapidity4l") or (obsName == 'dEtaj1j2')): qqzzTemplate = qqzzTempFile.Get("m4l_"+SuffixOfRootFile.replace("ZZ","zz").replace(".0","").replace('13000','3000'))
    if (ifLLR) and ((obsName == "rapidity4l") or (obsName == 'dEtaj1j2')): qqzzTemplate = qqzzTempFile.Get("m4l_"+SuffixOfRootFile.replace("ZZ","zz"))
    ...
    if (ifLLR): template_ggzzName = template_ggzzName.replace("ZZ","zz").replace('13000','3000')
    ggzzTempFile = TFile(template_ggzzName,"READ")
    if (not ifLLR): ggzzTemplate = ggzzTempFile.Get("m4l_"+SuffixOfRootFile)
    if (ifLLR) and not ((obsName == "rapidity4l") or (obsName == 'dEtaj1j2')): ggzzTemplate = ggzzTempFile.Get("m4l_"+SuffixOfRootFile.replace("ZZ","zz").replace(".0","").replace('13000','3000'))
    if (ifLLR) and ((obsName == "rapidity4l") or (obsName == 'dEtaj1j2')): ggzzTemplate = ggzzTempFile.Get("m4l_"+SuffixOfRootFile.replace("ZZ","zz"))
    ...
    if (ifLLR): template_zjetsName = template_zjetsName.replace("ZZ","zz").replace('13000','3000')
    zjetsTempFile = TFile(template_zjetsName,"READ")
    if (not ifLLR): zjetsTemplate = zjetsTempFile.Get("m4l_"+SuffixOfRootFile)
    if (ifLLR) and not ((obsName == "rapidity4l") or (obsName == 'dEtaj1j2')): zjetsTemplate = zjetsTempFile.Get("m4l_"+SuffixOfRootFile.replace("ZZ","zz").replace(".0","").replace('13000','3000'))
    if (ifLLR) and ((obsName == "rapidity4l") or (obsName == 'dEtaj1j2')): zjetsTemplate = zjetsTempFile.Get("m4l_"+SuffixOfRootFile.replace("ZZ","zz"))
    ...
    #elif (year == '2018'): data_obs_file = TFile('/eos/user/q/qguo/newNTuple_UL/2018/Slimmed_2p5/DataUL2018_all_noDuplicates_slimmed_newMuSF_add2p5.root')
   # if (year == '2016'): data_obs_file = TFile('/afs/cern.ch/user/t/tjavaid/public/data_UL2016_noDuplicates_created_LLRmissingevent_masked.root')
