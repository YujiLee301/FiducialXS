...
def parseOptions():
    ...
    # input options
    parser.add_option('',   '--obsName',dest='OBSNAME',    type='string',default='pT4l',   help='Name of the observalbe, supported: "mass4l", "pT4l", "massZ2", "rapidity4l", "cosThetaStar", "nets_reco_pt30_eta4p7"')
...
for obsName in observables:
    ...
    for obsbin in obsbins:
        ...
        if(opt.UNBLIND):
            #inFile = combineOutputs + "/higgsCombine"+obsName.replace(' ','_')+"_r_smH_"+obsbin+"_"+opt.ERA+unblindString+".MultiDimFit.mH125.38.root"
            inFile = combineOutputs + "/higgsCombine_"+obsName.replace(' ','_')+"_"+obsbin+".MultiDimFit.mH125.38.root"
            f_data = TFile(inFile,"READ")
            if (f_data==0): continue
        #inFile = combineOutputs + "/higgsCombine_"+obsName.replace(' ','_')+"_"+obsbin+".MultiDimFit.mH125.38.123456.root"
        inFile = combineOutputs + "/higgsCombine"+obsName.replace(' ','_')+"_"+obsbin+"_"+opt.ERA+unblindString+".MultiDimFit.mH125.38.root"
        ...
        if(opt.UNBLIND):
            for point in range(0, npoints_data):
                ...
                if (point == 0): bestfit_data=getattr(limit_data, obsbin.replace('r_smH_','r_smH_SM_125_'))
                if (point > 0):
                    if (limit_data.deltaNLL<2.5):
                        sigma_data.append(getattr(limit_data, obsbin.replace('r_smH_','r_smH_SM_125_')))
        ...
        for point in range(0, npoints_asimov):
            ...
            if (point == 0): bestfit=getattr(limit_asimov, obsbin.replace('r_smH_','r_smH_SM_125_'))
            if (point > 0):
                if (limit_asimov.deltaNLL<2.5):
                    sigma.append(getattr(limit_asimov, obsbin.replace('r_smH_','r_smH_SM_125_')))
        ...
        if(opt.UNBLIND):
            NoSystFile = combineOutputs + "/higgsCombine_"+obsName.replace(' ','_')+"_"+obsbin+"_NoSys.MultiDimFit.mH125.38.root"
            fstat_data = TFile(NoSystFile,"READ")
            if (fstat_data==0): continue
        #NoSystFile = combineOutputs + "/higgsCombine_"+obsName.replace(' ','_')+"_"+obsbin+"_NoSys.MultiDimFit.mH125.38.123456.root"
        NoSystFile = combineOutputs + "/higgsCombine"+obsName.replace(' ','_')+"_"+obsbin+"_NoSys"+"_"+opt.ERA+unblindString+".MultiDimFit.mH125.38.root"
        ...
        if(opt.UNBLIND):
            for point in range(0,npointsstat_data):
                ...
                if (point == 0): bestfitstat_data=getattr(limitstat_data, obsbin.replace('r_smH_','r_smH_SM_125_'))
                if (point > 0):
                    if (limitstat_data.deltaNLL<2.5):
                        sigmastat_data.append(getattr(limitstat_data, obsbin.replace('r_smH_','r_smH_SM_125_')))
        ...
        for point in range(0,npointsstat_asimov):
            ...
            if (point == 0): bestfit=getattr(limitstat_asimov, obsbin.replace('r_smH_','r_smH_SM_125_'))
            if (point > 0):
                if (limitstat_asimov.deltaNLL<2.5):
                    sigmastat.append(getattr(limitstat_asimov, obsbin.replace('r_smH_','r_smH_SM_125_')))
        ...
        print("obsName: {}, obsbin: {}".format(obsName,obsbin.replace('r_smH_','r_smH_SM_125_')))
        if(opt.UNBLIND):
        ...
        cl95.GetYaxis().SetTitle("-2 #Delta lnL")
        cl95.GetXaxis().SetTitle("#sigma_{fid.} [fb]")
        cl95.Draw("same")
        ...
        #index = obsbins.index(obsbin)
        #_temp = __import__('xsec_'+obsName, globals(), locals(), ['xsec'], -1)
        #SigmaBinH = _temp.xsec['SigmaBin'+str(index)]
        
        #All Ram's version removed below (Qianying's instead)

        """if(opt.UNBLIND):
            latex2.DrawLatex(0.20,0.72, "#sigma_{fid.}^{obs} = "+str(round(bestfit_data * SigmaBinH,2))+" ^{+"+str(round(cl68upstat_data * SigmaBinH, 2))+"}_{-"+str(round(cl68dnstat_data * SigmaBinH, 2))+"} (stat.) ^{+"+str(round(sysup_data * SigmaBinH, 2))+"}_{-"+str(round(sysdn_data * SigmaBinH, 2))+"} (sys.)")
        # else:
        latex2.SetTextColor(2)
        latex2.DrawLatex(0.20,0.80, "#sigma_{fid.}^{exp} = "+str(round(bestfit * SigmaBinH,2))+" ^{+"+str(round(cl68upstat * SigmaBinH, 2))+"}_{-"+str(round(cl68dnstat * SigmaBinH, 2))+"} (stat.) ^{+"+str(round( sysup * SigmaBinH, 2))+"}_{-"+str(round(sysdn * SigmaBinH, 2))+"} (sys.)")

        if (opt.UNBLIND):
            if (obsName=="mass4lPROBA"):
                #This needs to be generalised, will not work for mass4l, can't do it now - VM
                if (obsbin=="SigmaBin0"):
                ...
                resultsXS_data['SM_125_'+obsName.replace(' ','_')+'_genbin'+obsbin.replace('r_smH','')]                       = {"uncerDn": -1.0*cl68dn_data * SigmaBinH, "uncerUp": cl68up_data * SigmaBinH, "central": bestfit_data * SigmaBinH}
                resultsXS_data['SM_125_'+obsName.replace(' ','_')+'_genbin'+obsbin.replace('r_smH','')+'_statOnly']   = {"uncerDn": -1.0*cl68dnstat_data * SigmaBinH, "uncerUp": cl68upstat_data * SigmaBinH, "central": bestfit_data * SigmaBinH}
        ...
        if (obsName=="mass4lPROBA"):
            ...
            resultsXS['SM_125_'+obsName.replace(' ','_')+'_genbin'+obsbin.replace('r_smH','')]                       = {"uncerDn": -1.0*cl68dn * SigmaBinH, "uncerUp": cl68up * SigmaBinH, "central": bestfit * SigmaBinH}
            resultsXS['SM_125_'+obsName.replace(' ','_')+'_genbin'+obsbin.replace('r_smH','')+'_statOnly']   = {"uncerDn": -1.0*cl68dnstat * SigmaBinH, "uncerUp": cl68upstat * SigmaBinH, "central": bestfit * SigmaBinH}
        ...
        if (obsName=="mass4lPROBA"):
            ...
            with open(datacardInputs+'/resultsXS_LHScan_observed_'+obsName.replace(' ','_')+'_v3.py', 'w') as f:
                f.write('resultsXS = '+str(resultsXS_data)+' \n')
        #binCounter += 1
        if (obsName!='mass4lPROBA'): binCounter += 1"""
