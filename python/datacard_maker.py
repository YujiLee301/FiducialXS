...
parser.add_argument( '-m', dest= "physicsModel", default = "v2", help='Which physics model to use. Important for the naming of the signal process.')
...
def DataCardMaker(process_names, process_rate, nbins, current_bin, channel, observation, nuisances_name, nuisances_applied_to, nuisances_type, nuisances_value, path_dir, doZZ= False):
	    ...
	    f.write("jmax *"+"\n")
            ...
            for i in range(nbins):
                ...
                #tmp_line = tmp_line + process_names[0]+" "
...
print (process_names)
if physicsModel == 'v3': #VM: Be careful to check which processes should have this change, for this case it works although only trueH should be replaced.
    process_names = [p.replace('4e', '').replace('4mu', '').replace('2e2mu', '') for p in process_names]
    print (process_names)
