...
def extractResults(obsName, observableBins, modelName, physicalModel, asimovModelName, asimovPhysicalModel, resultsXS, year = 2018):
    ...
    print("yuji's upper limit checkpoint started")
    # Run the Combine
    if (physicalModel=="v3"):
        if (not opt.FIXFRAC):
            if (opt.FIXMASS=="False"):
            ...
            cmd = cmd + ' --algo=singles --cl=0.68 --robustFit=1'
            print("yuji's upper limit checkpoint A")
            output=processCmd(cmd, get_linenumber(), os.path.basename(__file__))
            print("yuji's upper limit checkpoint B")
    ...
    if (opt.SYS):
       ...
       #VM Test:  cmd = cmd + ' --freezeNuisanceGroups CMS_fakeH_p1_1_8,CMS_fakeH_p1_2_8,CMS_fakeH_p1_3_8,CMS_fakeH_p3_1_8,CMS_fakeH_p3_2_8,CMS_fakeH_p3_3_8 --freezeParameters allConstrainedNuisances'
        ...
        output = processCmd(cmd, get_linenumber(), os.path.basename(__file__))
        print("yuji's upper limit checkpoint endedC")
...
def runFiducialXS():
    ...
    ##LLR structure for v3
    if not(os.path.exists('./combine_files/')):
        os.mkdir('./combine_files/')
    ...
    asimovPhysicalModel = "v2" # FIXME: Same above message. yuji modified

    # FIXME: Why for mass4l we are using two versions while for others only v3.
    if (obsName == "mass4l"): physicalModels = ["v3"]#TEST physicalModels = ["v2","v3"]
    else: physicalModels = ["v3"]# FIXME: yuji modified
    ...
    if ((runAllSteps or opt.templatesOnly) and year.isdigit()): # Don't run this for all year. "year.isdigit()" is added to skip when "allYear" option is provided
        #test LLR
        #if not((obsName == "pT4l") or (obsName == "njets_pt30_eta4p7") or (obsName == "rapidity4l") or (obsName == "mass4l") or (obsName == "mj1j2") or (obsName == "pt_leadingjet_pt30_eta4p7") or (obsName == 'pTj2') or (obsName == 'dEtaj1j2') or (obsName == 'mass4lj') or (obsName == 'pT4lj') or (obsName == 'pT4ljj') or (obsName == 'TauB_Inc_0j_pTWgt') or (obsName == 'TauC_Inc_0j_EnergyWgt')): 
        extractBackgroundTemplatesAndFractions(obsName, observableBins, year, obs_ifJES, obs_ifJES2)

    logger.debug("resultsXS: {}".format(resultsXS))
    if (year == "allYear") and (obsName == "mass4l"):
    ...
    if ((runAllSteps or opt.resultsOnly)):
        for physicalModel in physicalModels:
            for modelName in modelNames:
                #if physicalModel == 'v3':
                #    if not (opt.ERA == "allYear"):
                #        produceDatacards(obsName, observableBins, modelName, physicalModel, obs_ifJES, obs_ifJES2, obs_ifAbs, obs_ifAbs2, opt.ERA)
                #        sys.exit()
                #    runv3(years, observableBins, obsName, "SM_125", physicalModel, JES = ifJES, fStates=['4e', '4mu', '2e2mu'])
                #   break
                print("yuji debug in resultOnly")
    ...
    if (year == "allYear"):
       ...
       #TESTING VM adding w+ instead of w - was crashing

        print ("Sada smo u folderu")
        print(os.getcwd())
        if "v3" in physicalModels: 
            os.chdir('../')
    ...
    #FIXME:yuji debug testing
    
    #if ('v3' in physicalModels): sys.exit()
    ...
    if (runAllSteps or opt.finalplotsOnly):
        ...
        if (obsName=="mass4l"):
            ...
            print("yuji checking higgs combine")
            for obsBin in range(0,nBins):
                ...
                if ((obsName=='mass4l' or obsName=='dEtaj1j2') and str(obsBin)=='0'): cmd = cmd.replace('0.0,3.0','0.0,5.0')
